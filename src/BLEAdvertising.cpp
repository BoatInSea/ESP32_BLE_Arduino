 ...
 *
 */
static uint8_t resp_manufacturer_data[] = {0x01, 0x01, 0xfe, 0x01, 0x01, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55};
BLEAdvertising::BLEAdvertising() {
	...
	m_advDataRsp.set_scan_rsp        = true;
	m_advDataRsp.include_name        = false;
	m_advDataRsp.include_txpower     = false;
	m_advDataRsp.min_interval        = 0x20;
	m_advDataRsp.max_interval        = 0x40;
	m_advDataRsp.appearance          = 0x00;
	m_advDataRsp.manufacturer_len    = 0;
	m_advDataRsp.p_manufacturer_data = nullptr;
	m_advDataRsp.service_data_len    = 0;
	m_advDataRsp.p_service_data      = nullptr;
	m_advDataRsp.service_uuid_len    = 0;
	m_advDataRsp.p_service_uuid      = nullptr;
	m_advDataRsp.flag                = (ESP_BLE_ADV_FLAG_GEN_DISC | ESP_BLE_ADV_FLAG_BREDR_NOT_SPT);
...
void BLEAdvertising::setManufacturer(uint8_t* man, uint8_t len) {
    m_advData.manufacturer_len = len;
    m_advData.p_manufacturer_data = man;
    memcpy(resp_manufacturer_data+5, man+4, 6);
}
...
void BLEAdvertising::start() {
	...
	if (numServices > 0) {
		...
		for (int i=0; i<numServices; i++) {
			...
			serviceUUID128 = m_serviceUUIDs[i].to128();
    ...
    //m_advDataRsp = m_advData;
    //m_advDataRsp.service_uuid_len = 16;
	//m_advDataRsp.set_scan_rsp = true;
    //m_advDataRsp.p_service_uuid = new uint8_t[m_advData.service_uuid_len];
	//memcpy(m_advDataRsp.p_service_uuid, serviceUUID128.getNative()->uuid.uuid128, 16);
    //m_advDataRsp.p_manufacturer_data = resp_manufacturer_data;
    //m_advDataRsp.manufacturer_len = 11; 
	//errRc = ::esp_ble_gap_config_adv_data(&m_advDataRsp);
	//if (errRc != ESP_OK) {
	...
	//if (m_advDataRsp.service_uuid_len > 0) {
	//	delete[] m_advDataRsp.p_service_uuid;
	//	m_advDataRsp.p_service_uuid = nullptr;
	//}
